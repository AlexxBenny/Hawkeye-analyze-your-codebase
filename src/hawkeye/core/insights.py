...
All numeric thresholds flow through ThresholdConfig — no hardcoded
magic numbers in any function.
"""
...
# ── Default thresholds (backward compat for direct imports) ───
...
# Legacy alias — tests import this; kept as a read-only view
# of the default profile so nothing breaks.
_THRESHOLDS = {
...
def _derive_coupling(ca: int, ce: int, t: "ThresholdConfig") -> list[Insight]:
...
def _derive_size(loc: int, t: "ThresholdConfig") -> list[Insight]:
...
def _derive_cycle(
    ...
    if cycle_count > 0:
        results.append(Insight(
            code="in_cycle",
            severity="critical" if max_cycle_size >= t.cycle_size_high else "warning",
            metric="cycle_count",
            value=cycle_count,
            threshold=1,
...
def _derive_dependency_fan(ce: int, t: "ThresholdConfig") -> list[Insight]:
...
def _derive_zones(
    ...
    if distance >= t.distance_high:
        ...
        # Zone of Uselessness: abstract + unstable = unused
            ...
            results.append(Insight(
                code="zone_of_uselessness",
                severity="warning",
                metric="distance_main_seq",
                value=distance,
                threshold=t.distance_high,
                detail=(
                    f"D={distance:.2f}: abstract (A={abstractness:.2f}) "
                    f"+ unstable (I={instability:.2f}) — possibly unused abstractions"
                ),
            ))

    elif distance <= t.distance_low:
        results.append(Insight(
            code="well_balanced",
            severity="info",
            metric="distance_main_seq",
            value=distance,
            threshold=t.distance_low,
            detail=(
                f"D={distance:.2f}: on the main sequence "
...
def derive_module_insights(
    *,
    ...
    insights.extend(_derive_size(loc, t))
...
def classify_risk(
    *,
    ...
    All numeric decisions use ThresholdConfig — zero hardcoded numbers.
    """
    t = thresholds or _default_thresholds()
    ...
    high_complexity = cyclomatic >= t.cc_high or cognitive >= t.cog_high

    # Hub: many dependents + complex (central point of failure)
    if ca >= t.dependents_high and high_complexity:
        return "hub"

    # Fragile: complex + unstable (likely to break)
    if high_complexity and instability >= t.instability_high:
        return "fragile"

    # Volatile: high instability + many outgoing deps
    if instability >= t.instability_high and ce >= t.dependents_high:
        return "volatile"
